...
public class FileServiceImpl implements FileService {
    ...
    public String upload(MultipartFile file) {
        try {
            ...
            if ("local".equals(fileStorageConfig.getStorageType())) {
                ...
                System.out.println("文件大小: " + fileBytes.length);
            ...
            // [NEW] 上传后异步处理：解析文本 → 存 MinIO → 创建文档记录
            processFileAfterUpload(fileId, fileName, fileBytes);
    ...
    public String mergeChunk(String fileId, String fileName) {
        try {
            ...
            if ("local".equals(fileStorageConfig.getStorageType())) {
                ...
                // 上传合并后的文件
                try (InputStream inputStream = new ByteArrayInputStream(mergedBytes)) {
                    minioClient.putObject(
                            PutObjectArgs.builder()
                                    .bucket(fileStorageConfig.getBucketName())
                                    .object(storageFileName)
            ...
            // [NEW] 合并后也进行文本提取和文档创建
            if (mergedBytes != null) {
                processFileAfterUpload(fileId, fileName, mergedBytes);
            }
     ...
     */
    private void processFileAfterUpload(String fileId, String fileName, byte[] fileBytes) {
        try {
            String text = DocumentTextExtractor.extractText(fileBytes, fileName);
            if (text.isEmpty()) {
                System.out.println("文件内容为空或格式不支持，跳过文档创建: fileId=" + fileId);
                return;
            }
            ...
            String txtObjectKey = "document-content/" + fileId + ".txt";
