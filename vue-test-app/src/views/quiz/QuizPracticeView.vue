  ...
  <div class="quiz-page" v-loading="loading">
    ...
    <div class="quiz-layout">
      <aside class="set-sidebar">
        ...
        <div v-for="set in savedSets" :key="set.id" class="saved-set" :class="{ active: activeSet?.id === set.id }" role="button" tabindex="0" @click="openSet(set.id)" @keyup.enter="openSet(set.id)"><span class="set-icon"><el-icon><Collection /></el-icon></span><span><strong>{{ set.title }}</strong><small>{{ formatDate(set.updateTime || set.createTime) }}</small></span><button class="delete-set" title="删除练题集" @click.stop="deleteSet(set)"><el-icon><Delete /></el-icon></button><el-icon><ArrowRight /></el-icon></div>
      ...
      <main class="quiz-main">
        ...
        <section v-else class="practice-card">
          <div class="practice-top"><div><button class="plain-back" @click="leaveSet"><el-icon><ArrowLeft /></el-icon> 练题集</button><h2>{{ activeSet.title }}</h2><p>{{ activeSet.sourceNames?.join('、') || '已保存练题集' }}</p></div><div class="practice-actions"><div class="score-chip">已答 {{ answeredCount }} · 正确 {{ correctCount }} · {{ accuracy }}%</div><el-button size="small" plain @click="exportReport">导出报告</el-button><el-button size="small" plain type="danger" @click="resetProgress">重置进度</el-button></div></div>
          ...
          <div class="practice-filters">
            <el-select v-model="practiceMode" size="small" style="width: 126px"><el-option label="全部题目" value="all" /><el-option label="未作答" value="unanswered" /><el-option label="错题本" value="wrong" /><el-option label="我的收藏" value="favorite" /></el-select>
            <el-select v-model="sourceFilter" clearable size="small" placeholder="来源文档" style="width: 180px"><el-option v-for="source in questionSources" :key="source" :label="source" :value="source" /></el-select>
            <el-select v-model="difficultyFilter" clearable size="small" placeholder="难度" style="width: 110px"><el-option label="基础" value="easy" /><el-option label="进阶" value="medium" /><el-option label="挑战" value="hard" /></el-select>
            <span class="progress-label">完成 {{ completionRate }}%</span><el-progress :percentage="completionRate" :show-text="false" />
          </div>
          <div v-if="answeredCount && answeredCount === activeSet.questions.length" class="completion-banner">本轮练习已完成：正确率 {{ accuracy }}%，错题 {{ wrongCount }} 道。可切换到“错题本”集中复习。</div>
          ...
          <article v-if="activeQuestion" class="question-card">
            <div class="question-meta"><div><span class="type-badge">{{ typeName(activeQuestion.type) }}</span><span class="difficulty-badge">{{ difficultyName(activeQuestion.difficulty) }}</span><span class="source-badge">{{ activeQuestion.sourceName || '未标注来源' }}</span></div><div><button class="favorite-button" :class="{ active: favoriteIds.includes(activeQuestion.id) }" @click="toggleFavorite(activeQuestion.id)">{{ favoriteIds.includes(activeQuestion.id) ? '★ 已收藏' : '☆ 收藏' }}</button><span>第 {{ activeIndex + 1 }} / {{ visibleQuestions.length }} 题</span></div></div>
...
const questionSources = computed(() => [...new Set((activeSet.value?.questions || []).map(question => question.sourceName || '未标注来源'))])
const visibleQuestions = computed(() => {
  let questions = activeSet.value?.questions || []
  if (activeFilter.value !== 'all') questions = questions.filter(question => question.type === activeFilter.value)
  if (sourceFilter.value) questions = questions.filter(question => (question.sourceName || '未标注来源') === sourceFilter.value)
  if (difficultyFilter.value) questions = questions.filter(question => question.difficulty === difficultyFilter.value)
  if (practiceMode.value === 'unanswered') questions = questions.filter(question => !feedbackById.value[question.id])
  if (practiceMode.value === 'wrong') questions = questions.filter(question => feedbackById.value[question.id] && !feedbackById.value[question.id].correct)
  if (practiceMode.value === 'favorite') questions = questions.filter(question => favoriteIds.value.includes(question.id))
  return questions
})
const activeQuestion = computed(() => visibleQuestions.value[activeIndex.value])
const answeredCount = computed(() => Object.keys(feedbackById.value).length)
...
const completionRate = computed(() => activeSet.value?.questions?.length ? Math.round(answeredCount.value * 100 / activeSet.value.questions.length) : 0)
...
function difficultyName(value) { return ({ easy: '基础', medium: '进阶', hard: '挑战' })[value] || '进阶' }
function inferDifficulty(question) { if (question.type === 'short' || String(question.stem || '').length > 90) return 'hard'; if (question.type === 'multiple' || String(question.stem || '').length > 45) return 'medium'; return 'easy' }
function normalize(value) { return String(value ?? '').toLowerCase().replace(/[\s\p{P}\p{S}]/gu, '') }
function startNewSet() { activeSet.value = null; activeIndex.value = 0; activeFilter.value = 'all'; practiceMode.value = 'all'; sourceFilter.value = ''; difficultyFilter.value = ''; answerSheet.value = {}; feedbackById.value = {}; favoriteIds.value = []; router.push('/quiz') }
...
function normalizeQuestion(raw, index) {
  ...
  const normalized = { id: String(raw.id || `q-${index + 1}`), type, stem: String(raw.stem || raw.question || raw.title || '').trim(), options: raw.options || options, answer: Array.isArray(answer) ? answer.map(String) : String(answer), analysis: String(raw.analysis || raw.explanation || '请结合参考答案复习对应知识点。'), sourceName: String(raw.sourceName || '未标注来源'), sourceDocumentId: raw.sourceDocumentId ? String(raw.sourceDocumentId) : '', difficulty: raw.difficulty || '' }
  normalized.difficulty = normalized.difficulty || inferDifficulty(normalized)
  return normalized
}
...
function practiceStorageKey(id = activeSet.value?.id) { return id ? `smartdoc_quiz_progress_${userId() || 'guest'}_${id}` : '' }
function loadPracticeState(id) {
  try {
    const saved = JSON.parse(localStorage.getItem(practiceStorageKey(id)) || '{}')
    answerSheet.value = saved.answerSheet || {}
    feedbackById.value = saved.feedbackById || {}
    favoriteIds.value = Array.isArray(saved.favoriteIds) ? saved.favoriteIds : []
  } catch { answerSheet.value = {}; feedbackById.value = {}; favoriteIds.value = [] }
}
function persistPracticeState() {
  const key = practiceStorageKey()
  if (!key) return
  localStorage.setItem(key, JSON.stringify({ answerSheet: answerSheet.value, feedbackById: feedbackById.value, favoriteIds: favoriteIds.value, updatedAt: new Date().toISOString() }))
}
function toggleFavorite(questionId) {
  favoriteIds.value = favoriteIds.value.includes(questionId) ? favoriteIds.value.filter(id => id !== questionId) : [...favoriteIds.value, questionId]
}
async function resetProgress() {
  try {
    await ElMessageBox.confirm('确定清空本练题集的答案、错题和收藏记录吗？题目不会被删除。', '重置练习进度', { confirmButtonText: '重置', cancelButtonText: '取消', type: 'warning' })
    answerSheet.value = {}; feedbackById.value = {}; favoriteIds.value = []; activeIndex.value = 0; practiceMode.value = 'all'
...
function exportReport() {
  ...
  const wrong = activeSet.value.questions.filter(question => feedbackById.value[question.id] && !feedbackById.value[question.id].correct)
  const lines = [`# ${activeSet.value.title} - 练习报告`, '', `- 导出时间：${new Date().toLocaleString()}`, `- 题目总数：${activeSet.value.questions.length}`, `- 已答：${answeredCount.value}`, `- 正确：${correctCount.value}`, `- 错题：${wrongCount.value}`, `- 正确率：${accuracy.value}%`, '', '## 错题明细', '']
  wrong.forEach((question, index) => { lines.push(`${index + 1}. ${question.stem}`, `   - 我的答案：${Array.isArray(answerSheet.value[question.id]) ? answerSheet.value[question.id].join('、') : answerSheet.value[question.id] || '未作答'}`, `   - 正确答案：${displayAnswer(question)}`, `   - 来源：${question.sourceName || '未标注来源'}`, '') })
  if (!wrong.length) lines.push('本次暂无错题。')
  const blob = new Blob([lines.join('\n')], { type: 'text/markdown;charset=utf-8' })
  const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `${activeSet.value.title}-练习报告.md`; link.click(); setTimeout(() => URL.revokeObjectURL(url), 500)
  ElMessage.success('练习报告已导出')
}
...
async function openSet(id) {
  ...
  try {
    ...
    activeIndex.value = 0; activeFilter.value = 'all'; practiceMode.value = 'all'; sourceFilter.value = ''; difficultyFilter.value = ''; loadPracticeState(String(id))
...
async function deleteSet(set) {
  try {
    await ElMessageBox.confirm(`确定删除练题集“${set.title}”吗？原始题库文档不会被删除。`, '删除练题集', { confirmButtonText: '删除', cancelButtonText: '取消', type: 'warning' })
    await docApi.deleteDoc(set.id)
    localStorage.removeItem(practiceStorageKey(String(set.id)))
    if (activeSet.value?.id === String(set.id)) { activeSet.value = null; router.push('/quiz') }
    await loadDocuments()
    ElMessage.success('练题集已删除')
  } catch (error) {
    if (error !== 'cancel' && error !== 'close') ElMessage.error(error?.message || '删除练题集失败')
  }
}
...
async function createQuizSet() {
  ...
  try {
    ...
    for (const source of sources) {
      const extracted = extractQuestionAnswerPairs(source.content, source.name, offset).map(question => ({ ...question, sourceName: source.name, sourceDocumentId: source.id, difficulty: question.difficulty || inferDifficulty(question) }))
    ...
    if (!questions.length) {
      ...
      questions = parseModelQuestions(generated).map(question => ({ ...question, sourceName: 'AI 综合题', difficulty: question.difficulty || inferDifficulty(question) }))
    }
...
function localShortAnswerScore(expected, actual) {
  const tokenize = value => {
    const text = String(value || '').toLowerCase()
    const tokens = text.replace(/[^p{L}p{N}]+/gu, ' ').split(/s+/).filter(token => token.length > 1)
    const chinese = (text.match(/[㐀-鿿]/g) || []).join('')
    for (let index = 0; index < chinese.length - 1; index++) tokens.push(chinese.slice(index, index + 2))
    return new Set(tokens)
  }
  const expectedTokens = tokenize(expected); const actualTokens = tokenize(actual)
  if (!expectedTokens.size || !actualTokens.size) return 0
  let overlap = 0; actualTokens.forEach(token => { if (expectedTokens.has(token)) overlap++ })
  return Math.min(100, Math.round((overlap * 2 / (expectedTokens.size + actualTokens.size)) * 100))
}
async function submitAnswer() {
  ...
  try {
    ...
    const score = localShortAnswerScore(question.answer, answer)
    feedbackById.value = { ...feedbackById.value, [question.id]: { correct: score >= 70, score, message: 'AI 判定暂不可用，已使用关键词相似度降级评分（' + (error?.message || '服务异常') + '）。' } }
    ElMessage.warning('AI 判定暂不可用，已完成本地降级评分')
  } finally { judging.value = false }
}
watch([activeFilter, practiceMode, sourceFilter, difficultyFilter], () => { activeIndex.value = 0 })
watch(() => visibleQuestions.value.length, length => { if (activeIndex.value >= length) activeIndex.value = Math.max(0, length - 1) })
watch([answerSheet, feedbackById, favoriteIds], persistPracticeState, { deep: true })
