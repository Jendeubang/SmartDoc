...
let documentSummaryTimer = null
onBeforeUnmount(() => {
  if (documentSummaryTimer) window.clearTimeout(documentSummaryTimer)
  documentSummaryTimer = null
  clearImagePreview()
  releaseParagraphLock()
})
...
const loadDocData = async () => {
  ...
  try {
    ...
    if (data.content && data.content.length > 5) {
      ...
      aiApi.analyzeText(plainText, { suppressGlobalError: true }).then(res => {
    ...
    if (data.content && data.content.length > 5) {
      ...
      aiApi.extractKeywords(data.content, 5, currentModel.value, { suppressGlobalError: true }).then(res => {
        keywords.value = res.data.keywords.map(k => k.word)
      }).catch(error => {
        console.warn('关键词提取接口调用失败，跳过自动关键词刷新', error)
...
onMounted(() => {
  ...
  loadDocData().then(() => {
    ...
    // loadDocData 已经提交过一次关键词分析，不再重复请求。
    // 摘要延迟执行，避免打开文档时与详情、模型和关键词请求同时冲击网关。
    if (!isChatMode.value && !aiSummary.value) {
      documentSummaryTimer = window.setTimeout(() => {
        const paperElement = document.querySelector('.paper')
        if (paperElement?.innerHTML) generateDocSummary(paperElement.innerHTML)
        documentSummaryTimer = null
      }, 1200)
    }
  })
...
const refreshKeywords = async (content) => {
  ...
  try {
    ...
    const res = await aiApi.extractKeywords(plainText, 5, currentModel.value, { suppressGlobalError: true })
...
const generateDocSummary = async (content) => {
  ...
  try {
    const plainText = content.replace(/<[^>]+>/g, '').trim()
    const res = await aiApi.summarizeText(plainText, 200, currentModel.value, { suppressGlobalError: true })
    if (res?.data?.summary) {
