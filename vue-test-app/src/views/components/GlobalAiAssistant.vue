...
const { currentStep: progressStep, startTracking, stopTracking } = useAgentProgress()
...
const handleSend = async () => {
  ...
  startTracking(null, uid)

  try {
    // 如果没有对话ID，executeTask 会自动创建，这里不提前调用 startChat
    // 等 executeTask 返回后，从结果中取 conversationId 保存
    ...
    if (agentResult.conversationId) {
      saveConvId(agentResult.conversationId)
    }
    ...
    let answerText = agentResult?.answer || agentResult?.text || "指令已执行完毕。"
    // 去除回答中的来源标记
