    ...
    <el-row :gutter="20" class="detail-row">
      <el-col :span="12">
        <el-card shadow="hover" class="section-card">
          ...
          <div class="window-switcher">
            <el-radio-group v-model="activeWindow" size="small" @change="switchWindow">
              <el-radio-button v-for="opt in windowOptions" :key="opt.value" :value="opt.value">
                {{ opt.label }}
              </el-radio-button>
            </el-radio-group>
          </div>
          <el-table :data="metricsTableData" stripe size="small" empty-text="暂无监控数据">
            <el-table-column prop="label" label="指标名称" />
      ...
      <el-col :span="12">
        <el-card shadow="hover" class="section-card">
          ...
          <el-button type="primary" size="small" style="margin-bottom: 12px" @click="detectFaults" :loading="loading">
...
const faults = ref([])
const activeWindow = ref('1m')
const windowOptions = [
  { label: '1 分钟窗口', value: '1m' },
  { label: '5 分钟窗口', value: '5m' },
  { label: '15 分钟窗口', value: '15m' }
]

const metricsTableData = computed(() => {
  const data = metrics.value
  return [
    { name: 'ai.requests.count', label: 'AI 请求总数', value: data['ai.requests.count'] ?? 0 },
    { name: 'ai.errors.count', label: 'AI 错误数', value: data['ai.errors.count'] ?? 0 },
    { name: 'ai.errorRate', label: '错误率', value: (data['ai.errorRate'] ?? 0) + '%' },
    { name: 'ai.request.stats.count', label: '请求统计-次数', value: data['ai.request.stats']?.count ?? 0 },
    { name: 'ai.request.stats.min', label: '请求统计-最短耗时', value: (data['ai.request.stats']?.min ?? 0) + 'ms' },
    { name: 'ai.request.stats.max', label: '请求统计-最长耗时', value: (data['ai.request.stats']?.max ?? 0) + 'ms' },
    { name: 'ai.request.stats.p95', label: '请求统计-P95', value: (data['ai.request.stats']?.p95 ?? 0) + 'ms' },
    { name: 'ai.request.stats.p99', label: '请求统计-P99', value: (data['ai.request.stats']?.p99 ?? 0) + 'ms' }
  ]
})
...
const formatWindowLabel = (win) => {
  const map = { '1m': '1 分钟', '5m': '5 分钟', '15m': '15 分钟' }
  return map[win] || win
}
...
const fetchMetrics = async () => {
  try {
    ...
    const raw = res.data || {}
    // 后端返回 { windows: { 1m: {...}, 5m: {...}, 15m: {...} } }
    // 展平当前选中的窗口到顶层
    const winData = raw.windows?.[activeWindow.value] || {}
    metrics.value = { ...winData, _raw: raw }
  } catch {
    ElMessage.error('获取监控指标失败')
  }
}

const switchWindow = (win) => {
