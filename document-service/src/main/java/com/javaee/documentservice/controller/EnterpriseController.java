...
public class EnterpriseController {
    ...
    public Result<Map<String, Object>> overview(@PathVariable String id) {
        ...
        if (enterpriseAdmin) {
            result.put("members", jdbc.queryForList("SELECT * FROM enterprise_member WHERE organization_id=? AND status='active' ORDER BY create_time", id));
        } else {
            result.put("members", jdbc.queryForList("SELECT * FROM enterprise_member WHERE organization_id=? AND status='active' AND department_id=? ORDER BY create_time", id, emptyToNull(departmentId)));
        }
        if (enterpriseAdmin) {
            result.put("folders", jdbc.queryForList("SELECT * FROM enterprise_folder WHERE organization_id=? ORDER BY create_time", id));
            result.put("documents", jdbc.queryForList("SELECT id,title,user_id,organization_id,department_id,folder_id,status,update_time FROM document WHERE organization_id=? AND status='active' ORDER BY update_time DESC", id));
        } else {
            ...
            result.put("documents", jdbc.queryForList("SELECT id,title,user_id,organization_id,department_id,folder_id,status,update_time FROM document WHERE organization_id=? AND status='active' AND (department_id IS NULL OR department_id=?) ORDER BY update_time DESC", id, emptyToNull(departmentId)));
        }
    ...
    public Result<Map<String, Object>> addMember(@PathVariable String id, @RequestBody Map<String, Object> body) {
        ...
        notifyUser(memberUserId, "ENTERPRISE_INVITE", "你已加入企业空间", "企业管理员已将你加入文档空间", id);
    ...
    public Result<Void> moveDocument(@PathVariable String documentId, @RequestBody Map<String, Object> body) {
        Long userId = users.getRequiredUserId(); Document doc = requiredDocument(documentId); documentAccess.assertCanWrite(doc, userId);
        ...
        jdbc.update("UPDATE document SET organization_id=?,department_id=?,folder_id=?,update_time=NOW() WHERE id=?",
                organizationId, emptyToNull(departmentId), emptyToNull(folderId), documentId);
    ...
    public Result<Map<String, Object>> submitApproval(@RequestBody Map<String, Object> body) {
        ...
        notifyUser(approver, "APPROVAL_PENDING", "收到新的文档审批", doc.getTitle(), id);
    ...
    public Result<List<Map<String, Object>>> approvals() {
        Long userId = users.getRequiredUserId();
        return Result.success(jdbc.queryForList("SELECT a.*,d.title document_title FROM document_approval a LEFT JOIN document d ON d.id=a.document_id WHERE a.approver_user_id=? OR a.applicant_user_id=? ORDER BY a.create_time DESC", userId, userId));
    }
    ...
    public Result<Void> decide(@PathVariable String id, @RequestBody Map<String, Object> body) {
        ...
        notifyUser(((Number) approval.get("applicant_user_id")).longValue(), "APPROVAL_DECISION", "文档审批已处理", decision, id);
    ...
    public Result<List<Map<String, Object>>> audits(@PathVariable String id) {
        Long userId = users.getRequiredUserId(); permissions.assertEnterpriseAdmin(id, userId);
        return Result.success(jdbc.queryForList("SELECT * FROM enterprise_audit_log WHERE organization_id=? ORDER BY create_time DESC LIMIT 300", id));
    }
    ...
    public Result<Map<String, Object>> statistics(@PathVariable String id) {
        ...
        result.put("topUsers", jdbc.queryForList("""
            SELECT user_id,COUNT(*) operations FROM enterprise_audit_log
            WHERE organization_id=? AND create_time>=DATE_SUB(NOW(),INTERVAL 30 DAY)
            GROUP BY user_id ORDER BY operations DESC LIMIT 8
            """, id));
        return Result.success(result);
    }
    ...
    private void notifyUser(Long userId, String type, String title, String content, String relatedId) { jdbc.update("INSERT INTO enterprise_notification(id,user_id,type,title,content,related_id) VALUES(?,?,?,?,?,?)", UUID.randomUUID().toString(), userId, type, title, content, relatedId); }
    private void ensureUser(Long userId) { try { Map<?,?> response = rest.getForObject(userServiceUrl + "/api/users/" + userId, Map.class); if (response == null || !"200".equals(String.valueOf(response.get("code")))) throw new BusinessException("用户不存在"); } catch (BusinessException e) { throw e; } catch (Exception e) { throw new BusinessException("用户不存在或用户服务不可用"); } }
}
