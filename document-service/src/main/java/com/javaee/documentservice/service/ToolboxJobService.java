...
public class ToolboxJobService {
    ...
    private final Map<String, Map<String,Object>> jobs = new ConcurrentHashMap<>(); private final Map<String, byte[]> files = new ConcurrentHashMap<>();
    ...
    public Map<String,Object> submit(ToolboxJobRequest r, Long userId) {
        ...
        Map<String,Object> j=new ConcurrentHashMap<>(); j.put("jobId",id);j.put("userId",userId);j.put("toolType",type);j.put("documentIds",r.getDocumentIds());j.put("pages",r.getPages());j.put("status","PENDING");j.put("progress",5);j.put("message","任务已进入队列");j.put("createdAt",Instant.now().toString());jobs.put(id,j);rabbit.convertAndSend(EXCHANGE,KEY,id);return snapshot(id,userId);
    }
    ...
    private byte[] download(DocumentVO d){if(d.getFileId()==null||d.getFileId().isBlank())throw new BusinessException("该文档没有关联原始文件");return http.getForObject(fileUrl+"/api/files/download/"+d.getFileId(),byte[].class);}
