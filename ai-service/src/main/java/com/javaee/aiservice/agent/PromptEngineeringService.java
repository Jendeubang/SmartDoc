...
/**
 * 【简历：Prompt 工程服务】
 * 管理和构建 AI 对话的 Prompt 模板，支撑 RAG 问答、
 * Agent 对话等场景的系统提示词生成。
 */
...
public class PromptEngineeringService {
    ...
    private static final String CORRECT_TEMPLATE = """
            你是专业的文档校对助手。请对以下文本进行纠错：

            【原文】
            {content}

            要求：
            1. 逐条列出原文中的错别字、语病、标点错误、表达不当之处
            2. 每条格式：【类型】位置: "原文片段" 建议: "修改建议"
            3. 如果没有发现任何错误，回复"未发现明显问题"
            4. 纯文本输出，不要使用Markdown格式（不要用*、#、-等符号）
            """;
     ...
     * @return 提示词字符串
     */
    public String createCorrectPrompt(String content) {
        PromptTemplate template = new PromptTemplate(CORRECT_TEMPLATE);
        Map<String, Object> params = new HashMap<>();
        params.put("content", content);
        return template.render(params);
    }
