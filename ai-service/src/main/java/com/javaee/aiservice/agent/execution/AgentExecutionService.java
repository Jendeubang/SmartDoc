...
/**
 * 【简历：统一 AI Agent 执行链路】
 * 核心执行引擎：支持任务规划(buildIterationPlan)、工具调用、结果观察、
 * 反思重规划(reflectAfterIteration)和最终回答的多轮 Agent 执行闭环。
 * 兼容对话式 Agent 与 Plan-Execute Agent 场景。
 * 通过 Redis(AgentTaskRegistry)持久化任务状态，WebSocket(AgentProgressBroadcaster)实时推送进度。
 */
...
public class AgentExecutionService {
    ...
    private List<AgentPlanStep> fallbackPlan(AgentExecutionRequest request, Map<String, Object> context) {
        ...
        // 当 frontendDocumentWrite=true 时，生成两步计划：生成内容 → document-write 写回
    ...
    private AgentToolResult executeTextCorrect(Map<String, Object> params) {
        ...
        boolean isErrorCorrection = instruction.contains("纠错");
        String prompt;
        if (isErrorCorrection) {
            prompt = """
                    ...
                    请逐条列出原文中的错别字、语病或表达不当之处，纯文本格式（不要使用Markdown，不要用*、#、-等符号），不要修改原文，仅标注问题：
                    
                    问题1
                    位置: "原文中出问题的片段"
                    问题: 具体说明错误类型（错别字/语病/表达不当）
                    建议: 修改建议
                    
                    如果没有发现错误，请回复"未发现明显问题"。
            ...
            prompt = """
                    ...
                    要求：直接输出润色/改写后的完整文本（纯文本，不要使用Markdown格式，不要用*、#、-等符号），不要省略、不要加额外说明、不要自我介绍。
                    确保输出包含原文所有关键信息，只改进表达方式、用词准确性和流畅度。
                    """.formatted(instruction, content);
        }
