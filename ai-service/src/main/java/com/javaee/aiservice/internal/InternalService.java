...
public class InternalService {
    ...
    public void logAudit(String skillName, Map<String, Object> params, Map<String, Object> result) {
        ...
        if (mcpAuditLogMapper == null) return;
        try {
            McpAuditLog logEntry = new McpAuditLog();
            logEntry.setAction(skillName);
            logEntry.setUserId(String.valueOf(params.getOrDefault("userId", params.getOrDefault("userId", ""))));
            logEntry.setTraceId(String.valueOf(params.getOrDefault("traceId", "")));
            logEntry.setParams(objectMapper.writeValueAsString(params));
            logEntry.setResult(objectMapper.writeValueAsString(result));
            logEntry.setStatus(result != null && "error".equals(result.get("status")) ? "FAIL" : "SUCCESS");
            logEntry.setCreateTime(LocalDateTime.now());
            mcpAuditLogMapper.insert(logEntry);
        } catch (Exception e) {
            log.warn("审计日志写入失败", e);
        }
    }
