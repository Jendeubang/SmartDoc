...
class PermissionAwareRagServiceTest {
    ...
    @Test
    void passesAuthorizedDocumentIdsIntoRecallBeforeRerank() {
        KnowledgeBase knowledgeBase = mock(KnowledgeBase.class);
        DocumentServiceClient documents = mock(DocumentServiceClient.class);
        when(documents.getAccessibleDocumentIds()).thenReturn(List.of("doc-1", "doc-2"));
        when(knowledgeBase.hybridSearchWithRerank(any(), anyInt(),
        ...
        service.hybridSearchWithRerank("权限过滤", 5, "default", Reranker.RerankStrategy.HYBRID,
                DocumentSegmenter.StrategyType.CHAPTER);

        @SuppressWarnings("unchecked")
        ArgumentCaptor<Map<String, Object>> filters = ArgumentCaptor.forClass(Map.class);
        verify(knowledgeBase).hybridSearchWithRerank(any(), anyInt(),
                eq(Reranker.RerankStrategy.HYBRID), eq(DocumentSegmenter.StrategyType.CHAPTER), filters.capture());
        assertThat(filters.getValue().get("knowledgeBaseId")).isEqualTo("default");
        assertThat(filters.getValue().get("__allowedDocumentIds"))
                .isEqualTo(java.util.Set.of("doc-1", "doc-2"));
    }
}
